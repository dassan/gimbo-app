...
Em andamento:
- **F-29** — Saúde Financeira: tela `/health` **v1 (dívida + peso no orçamento) ligada aos motores reais** (HE-01 a HE-11 resolvidos: entidade `LOAN`, motor de dívida total/comprometido/horizonte, renda híbrida com override editável, detalhamento expansível real por cartão/`LOAN`, testes de componente). Dívida não-cartão lançada parcela a parcela (HE-15) resolvida (2026-06-25). Sessão 2026-06-26 reverteu o modelo de dois *backings*: `LOAN` passa a ser a entidade central de empréstimos longos, com motor de geração de transações reais (HE-19 a HE-22, resolvidas) — a marca leve "marcar série como empréstimo" (HE-16) foi removida por completo (tipo, schema, tabela SQLite, UI) em favor da `LOAN`, e um CTA em `TransactionDrawer.tsx` aponta parcelamentos longos para a seção Empréstimos; HE-17/HE-18 supersedidos. Reserva de Emergência segue mockada com selo "Em breve" (épico próprio). Ver `plan/FINANCIAL_HEALTH.md` §6-9.
