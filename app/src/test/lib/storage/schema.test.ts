...
describe('validateDataFile — v1 → v2 migration', () => {
  ...
  it('migrates a v1 file to schemaVersion 5 (current)', () => {
  ...
  it('accepts a v2 file and migrates it to schemaVersion 5', () => {
    const result = validateDataFile({ ...V1_FILE, schemaVersion: 2 })
    expect(result.schemaVersion).toBe(5)
  })

  it('migrates a v3 file to schemaVersion 5', () => {
    const result = validateDataFile({ ...V1_FILE, schemaVersion: 3, valuations: [] })
    expect(result.schemaVersion).toBe(5)
  })

  it('migrates a v4 file to schemaVersion 5', () => {
    const result = validateDataFile({ ...V1_FILE, schemaVersion: 4, valuations: [] })
    expect(result.schemaVersion).toBe(5)
  })

  it('accepts a v5 file without running migration (idempotent)', () => {
    const result = validateDataFile({ ...V1_FILE, schemaVersion: 5, valuations: [] })
    expect(result.schemaVersion).toBe(5)
  })

  it('throws SchemaVersionError for a v6 file (future version)', () => {
    expect(() => validateDataFile({ ...V1_FILE, schemaVersion: 6, valuations: [] })).toThrow(
      SchemaVersionError
    )
  })
})
...
describe('validateDataFile — v2 → v3 migration (NW-08)', () => {
  ...
  it('migrates a v2 file to schemaVersion 5 (current)', () => {
  ...
  it('createEmptyDataFile sets schemaVersion to 5', () => {
    const file = createEmptyDataFile('Test', 'test@example.com')
